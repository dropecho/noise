...
massive_munit_Assert.throws = function(expectedType,code,info) {
	try {
		...
		massive_munit_Assert.fail(tmp + c.__name__ + ": " + Std.string(e),{ fileName : "massive/munit/Assert.hx", lineNumber : 287, className : "massive.munit.Assert", methodName : "throws"});
	}
	return null;
};
...
massive_munit_TestRunner.prototype = {
	...
	,execute: function() {
		...
		if(!this.asyncPending) {
			var hrtime = process.hrtime();
			var time = hrtime[0] + hrtime[1] / 1e9 - this.startTime;
			var _g = 0;
			var _g1 = this.clients;
			while(_g < _g1.length) {
				var client = _g1[_g];
	...
	,executeTestCases: function() {
		...
		while(testCaseData.hasNext()) {
			...
			if(testCaseData1.result.ignore) {
				...
				var hrtime = process.hrtime();
				this.testStartTime = hrtime[0] + hrtime[1] / 1e9;
	...
	,executeTestCase: function(testCaseData,async) {
		...
		try {
			...
			if(async) {
				...
				result.passed = true;
				var hrtime = process.hrtime();
				result.executionTime = hrtime[0] + hrtime[1] / 1e9 - this.testStartTime;
				this.passCount++;
				var _g = 0;
				var _g1 = this.clients;
				while(_g < _g1.length) {
...
massive_munit_client_JUnitReportClient.prototype = {
	...
	,startTestSuite: function() {
		...
		var hrtime = process.hrtime();
		this.suiteExecutionTime = hrtime[0] + hrtime[1] / 1e9;
	...
	,endTestSuite: function() {
		...
		var hrtime = process.hrtime();
		this.suiteExecutionTime = hrtime[0] + hrtime[1] / 1e9 - this.suiteExecutionTime;
